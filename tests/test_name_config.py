...
def test_main_config(app_obj: AppObject):
    ...
    assert config.get_module_for_database('folder1/folder2/file') == '.folder1.folder2.file'
...
def test_wird_config(app_obj: AppObject):
    ...
    assert config.get_module_for_database('folder1/folder2/file') == '...folder1.folder2.file'
    assert config.get_module_for_database('database/folder2/a') == '.a'
    assert config.get_module_for_database('database/b/a') == '..b.a'
    ...
    config.update_config(schema_file='{class_name}/{name}/schema', base_schema_tmpl='{name}SomethingElse')
